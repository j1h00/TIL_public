...
# cost 가 최소화하면서, 필요한 메모리 M 바이트를 확보해야 한다. 
# memory 중에서 M 바이트 이상이 되도록 고른 뒤에, cost 가 최소인 것을 찾는다. 

# 냅색 => W 를 넘지 않도록 하고, 가치가 최대가 되도록
# 반대로, 가치가 일정값을 넘지 않도록 하고, 무게가 최대가 되도록 
# 이 문제에 적용해보면, cost 가 일정값을 넘지 않는 조건에서, memory 가 최대가 되도록

# dp[i][j] => i 번째 앱까지 고려했을 때, j 코스트로 얻을 수 있는 최대 memory
answer = 10**7
dp = [[0]*(sum(cost) + 1) for _ in range(N)]
for i in range(N):
    now_m = memory[i] 
    now_c = cost[i]
    for j in range(sum(cost) + 1):
        if j < now_c:
            ...
            dp[i][j] = max(memory[i] + dp[i-1][j-now_c], dp[i-1][j])
