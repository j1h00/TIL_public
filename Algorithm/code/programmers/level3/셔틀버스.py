...
# 셔틀은 9 시부터! t 분 간격으로 n 번 온다!
def solution(n, t, m, timetable):
    m_timetable = sorted(list(map(change_datetime, timetable)))
    # timetable 에 존재하는 크루들이, 어떤 버스를 타게 될지 정해보자 
    ...
    for time in bus_times:
        cnt = 0
        while i < len(m_timetable) and m_timetable[i] <= time and cnt < m:
            i += 1
            cnt += 1
        last_cnt = cnt 
    if last_cnt < m:
